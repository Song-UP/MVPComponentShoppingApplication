...
//    }
    ...
    public long isBankCards() {
        return checkBankCards(getBankCardText());
    }

    /**
     * 校验银行卡卡号
     */
    public long checkBankCards(String cardId) {
        if(cardId.length() > 8){
            long t = Long.parseLong(cardId.substring(0, 6));
            int pos = BankInfo.binarySearch(BankInfo.bankBin, t);
            if(pos != -1){
                return pos;
            }
        }
